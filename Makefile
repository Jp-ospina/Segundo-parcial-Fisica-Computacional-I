...
CXX = g++
CXXFLAGS = -Wall -O2 -I./punto1_pendulo_elastico/include -I./punto2_depredador_presa/include
...
depredador_presa:
	...
	@mkdir -p punto2_depredador_presa/results/graficas
	@$(CXX) $(CXXFLAGS) -o punto2_depredador_presa/depredador_presa.exe \
		punto2_depredador_presa/src/main.cpp \
		punto2_depredador_presa/src/depredador_presa.cpp \
		punto2_depredador_presa/src/rk4.cpp \
		punto2_depredador_presa/src/derivadas.cpp
	@echo -e "$(AZUL)► Ejecutando simulación...$(RESET)"
	@cd punto2_depredador_presa && ./depredador_presa.exe
...
graficar_depredador_presa:
	...
	read grafica; \
	if [ "$$grafica" = "1" ]; then \
		cd punto2_depredador_presa && gnuplot scripts/graficar_gnuplot.gp; \
		echo -e "$(VERDE)✔ Gráfica guardada en results/graficas/depredador_presa.png$(RESET)\n"; \
	elif [ "$$grafica" = "2" ]; then \
		cd punto2_depredador_presa && python3 scripts/graficar_python.py; \
	else \
		echo -e "$(ROJO)Opción inválida. No se graficó.$(RESET)\n"; \
	fi
...
clean:
	...
	@rm -f punto1_pendulo_elastico/pendulo.exe punto2_depredador_presa/depredador_presa.exe
	@rm -f punto1_pendulo_elastico/results/*.dat punto2_depredador_presa/results/*.dat
	@rm -rf punto1_pendulo_elastico/results/graficas/*.png punto2_depredador_presa/results/graficas/*.png
